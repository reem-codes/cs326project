...
from referit3d.models.losses import MultiheadLoss
...
if __name__ == '__main__':
    ...
    if args.s_vs_n_weight is not None:  # TODO - move to a better place
    	...
    	# instead of cross entropy, use multihead loss :)
        criteria['logits'] = MultiheadLoss(args.margin, weights={'ce': args.ce, 'triplet': args.triplet, 'contrastive': args.contrastive}).to(device)
        criteria['logits2'] = MultiheadLoss(args.margin, weights={'ce': args.ce2, 'triplet': args.triplet2, 'contrastive': args.contrastive2}).to(device)
