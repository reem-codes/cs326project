...
class ListeningDataset(Dataset):
    ...
    def get_reference_data(self, index):
        ...
        tokens = np.array(self.vocab.encode(ref['tokens'], self.max_seq_len), dtype=np.compat.long)
    ...
    def __getitem__(self, index):
        results = [] # will return two samples: anchor and positive
        ...
        if self.evalmode:
            ...
            ref = self.references.loc[index]
            pid = list(self.references[(self.references['scan_id'] == ref.scan_id) & (self.references['target_id'] == ref.target_id)].index)
            pid.remove(index)
            if len(pid) != 0:
                # this object has only one utterance, there are 18 of those only
                pid = random.choice(pid)
                scan, target, tokens, text_tokens, is_nr3d = self.get_reference_data(pid)
                ## BERT tokenize
                token_inds = torch.zeros(self.max_seq_len, dtype=torch.long)
                indices = self.bert_tokenizer.encode(
                    ' '.join(text_tokens), add_special_tokens=True)
                ...
                results[-1]['tokens'] = tokens
                results[-1]['token_inds'] = token_inds.numpy().astype(np.int64)
                results[-1]['token_num'] = token_num.numpy().astype(np.int64)
                results[-1]['is_nr3d'] = is_nr3d
		...
		# first result is the anchor
        results.append(res)
        results.append(copy.deepcopy(res)) # positive sample is a copy of anchor
        
        # then change the utterance only
        
        # get the index of the anchor
        ref = self.references.loc[index]
        # get another sample with the same scan id and target to copy its utterance
        pid = list(self.references[(self.references['scan_id'] == ref.scan_id) & (self.references['target_id'] == ref.target_id)].index)
        pid.remove(index)
        if len(pid) != 0:  # if it's zero, this object has only one utterance, there are 18 of those only
            pid = random.choice(pid) # choose another utterance randomly from the valid ones
            scan, target, tokens, text_tokens, is_nr3d = self.get_reference_data(pid)
            ## BERT tokenize
            token_inds = torch.zeros(self.max_seq_len, dtype=torch.long)
            indices = self.bert_tokenizer.encode(
