...
def single_epoch_train(model, data_loader, criteria, optimizer, device, pad_idx, args, epoch=None):
    ...
    for pairs in tqdm.tqdm(data_loader):
        # Move data to gpu
        for s in range(2):
            for k in batch_keys:
                if k in pairs[s]:
                    pairs[s][k] = pairs[s][k].to(device)
            if args.object_encoder == 'pnet':
                pairs[s]['objects'] = pairs[s]['objects'].permute(0, 1, 3, 2)

		# get anchor and positive sample
        batch = pairs[0]
        pos = pairs[1] 
        ...
        # Backward
        optimizer.zero_grad()
        all_losses = compute_losses(batch, res, criteria, args, pos)
...
def compute_losses(batch, res, criterion_dict, args, pos):
    ...
    if args.s_vs_n_weight is not None:
    	...
    	# the loss was changed from cross entropy to custom loss
        total_loss = criterion(logits, batch['target_pos'], res['mmt_obj_output'], pos['mmt_obj_output'])
    ...
    if args.context_2d=='unaligned':
        ...
        if args.s_vs_n_weight is not None:
            ...
            vg2d_loss = criterion_dict['logits2'](res['logits_2D'], batch['target_pos'], res['mmt_obj_output_2D'], pos['mmt_obj_output_2D'])
